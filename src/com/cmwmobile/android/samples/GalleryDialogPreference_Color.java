 ...
 */
public class GalleryDialogPreference_Color extends DialogPreference {
	// Layout widgets.
	 ...
	 */
	public GalleryDialogPreference_Color(Context context, AttributeSet attrs) {
		super(context, attrs);
			
		
		 
		Resources res = context.getResources();
		colorCodes = res.getStringArray(R.array.colorCodes);
		iconIds = new int[colorCodes.length];
		String packageName = context.getPackageName();
		for( int i = 0; i < colorCodes.length; i++ ) {
			iconIds[i] = res.getIdentifier("bg" + (colorCodes[i]).toLowerCase(), "drawable", packageName );
		}
	}
	/**
	...
	protected View onCreateDialogView() {
		...
		gallery.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				selectedColorCode = colorCodes[position];
			}
		});
